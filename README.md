...
* [x] https://www.4icu.org/
  ...
  * Implemented solution
    ...
    * `uniaz.py` is scraper, `urlList.json` is the list of links with is-processed status. `finalData.json` contain data for each university. This scraper update the local `.json` files after fetching each new 100 university data with is-processed status and new data. So, if scraper crashed fetched data will be safe and after running again it will start from the point where it stoped.
